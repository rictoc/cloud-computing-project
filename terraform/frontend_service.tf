resource "aws_launch_configuration" "frontend_launch_configuration" {
  ...
  user_data = <<-EOT
    ...
    docker pull \
    ${data.aws_caller_identity.current.account_id}.dkr.ecr.${data.aws_region.current.name}.amazonaws.com/cc-project/frontend
...
resource "aws_autoscaling_group" "frontend_asg" {
  ...
  tag {
    key                 = "Name"
    value               = "(cc-project) Frontend ASG instance"
